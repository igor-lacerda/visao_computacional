...
def _calculate_concat_dimensions(
    width_1: int, 
    height_1: int, 
    width_2: int, 
    height_2: int, 
    ...
    """
    Calcula as novas dimensões para a concatenação de imagens.
    Retorna o tamanho da segunda imagem e tamanho final das duas juntas: (size_2, final_size).
    """
    orientation = orientation.lower()
    if orientation not in ("horizontal", "vertical", "h", "v"):
        raise ValueError("Orientation deve ser 'horizontal' (h) ou 'vertical' (v)")
    ...
    if orientation in ("horizontal", "h"):
        ...
        if width_1 != width_2:
            ...
            size_2 = (width_1, new_height_2)
        else:
            size_2 = (width_2, height_2)
            
        final_size = (size_2[0], height_1 + size_2[1])

    return size_2, final_size
...
def _merge_frames(
    ...
    """
    Retorna a concatenação de duas imagens
    """
    # Redimensiona a imagem, se necessário
    if (frame_2.shape[1], frame_2.shape[0]) != size_2:
        frame_2 = cv2.resize(frame_2, size_2)

    if orientation.lower() in ("horizontal", "h"):
        return cv2.hconcat([frame_1, frame_2])
    else:
        return cv2.vconcat([frame_1, frame_2])
...
def concatenate_images(
    ...
    """
    Retorna uma imagem formada por outras duas de entrada.
    """
    height_1, width_1 = image_1.shape[:2]
    height_2, width_2 = image_2.shape[:2] 

    size_2, final_size = _calculate_concat_dimensions(width_1, height_1, width_2, height_2, orientation)

    return _merge_frames(image_1, image_2, size_2, orientation)
...
def concatenate_videos(
    ...
    try:
        while True:
            ...
            merged_frame = _merge_frames(frame_1, frame_2, size_2, orientation)
